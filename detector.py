    ...
    def get_localization(self, image, visual=False, threshold=0.4):  
        ...
        if len(idx_vec) ==0:
            ...
            if (len(filtered_boxes) != len(tmp_car_boxes)):
                print('Non-maximum suppression removed {} detections'.format(len(tmp_car_boxes) - len(filtered_boxes)))
...
if __name__ == '__main__':
    ...
    # images = glob(os.path.join('test_images/', '*.jpg'))
    # for i in images:
    #     image = cv2.cvtColor(cv2.imread(), cv2.COLOR_BGR2RGB)

    # result = detector.get_localization(image, 0.4)
    # print(result)
